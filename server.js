...
// GET endpoint to fetch status code information
app.get('/status-info', (req, res) => {
  const code = parseInt(req.query.code);

  if (!code || !statusCodeDescriptions[code]) {
    return res.status(400).json({
      error: "Invalid or unsupported status code. Please provide a valid code from the supported list."
    });
  }
